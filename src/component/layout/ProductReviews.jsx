...
const ProductReviews = ({ initialReviews = [], productId = null, onReviewAdded = null }) => {
...
const fetchProductReviews = async () => {
 ...
 useEffect(() => {
  ...
  useEffect(() => {
    ...
    // 2. Cancel any existing loop to prevent duplicates
    if (rafRef.current) cancelAnimationFrame(rafRef.current);

    // 3. Setup dimensions and initial position
    // We calculate these ONCE per setup, not every frame (better performance)
    const totalWidth = container.scrollWidth;
    const chunk = totalWidth / 3;

    // Guard against 0 width (element hidden or not laid out yet)
  ...
  initializedRef.current = true;
    ...
    // Attach global listeners for drag release
...
const handleSubmitReview = async () => {
  ...
  return (
    <section className="mt-10 py-2 sm:py-8 md:py-8 lg:py-8 relative overflow-hidden py-8 sm:py-12 md:py-16 lg:py-20 relative overflow-hidden bg-[#9B7BB8] rounded rounded-3xl">
      {/* Background Pattern */}
